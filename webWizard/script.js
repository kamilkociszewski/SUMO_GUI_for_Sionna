on("ready", function(){
    ...
    function updateBBoxInput() {
        ...
        var west = cor.left + width * canvasRect[0];
        var north = cor.top - height * canvasRect[1];
        var east = cor.left + width * canvasRect[2];
        var south = cor.top - height * canvasRect[3];
        ...
        elem("#bbox_input").value = bounds.left.toFixed(6) + "," + bounds.bottom.toFixed(6) + "," + bounds.right.toFixed(6) + "," + bounds.top.toFixed(6);
    }
    ...
    function setBBox() {
        var input = elem("#bbox_input").value;
        var coords = input.split(",");
        if (coords.length !== 4) {
            window.alert("Please enter coordinates as: west,south,east,north");
            return;
        }
        var west = parseFloat(coords[0]);
        var south = parseFloat(coords[1]);
        var east = parseFloat(coords[2]);
        var north = parseFloat(coords[3]);

        if (isNaN(west) || isNaN(south) || isNaN(east) || isNaN(north)) {
            window.alert("Invalid coordinates. Please enter numeric values.");
            return;
        }
        ...
        // calculate canvasRect relative to the new map extent
        var newExtent = map.getExtent();
        var width = newExtent.right - newExtent.left;
        var height = newExtent.top - newExtent.bottom;

        canvasRect[0] = (bounds.left - newExtent.left) / width;
        canvasRect[1] = (newExtent.top - bounds.top) / height;
        canvasRect[2] = (bounds.right - newExtent.left) / width;
        canvasRect[3] = (newExtent.top - bounds.bottom) / height;

        draw();

        // Update the input field with the original input to ensure consistency
        elem("#bbox_input").value = input;
    }

    elem("#buttonBBox").on("click", setBBox);
    elem("#bbox_input").on("keyup", function(e){
        if (e.keyCode == 13) {
            setBBox();
        }
    });
    ...
    map.events.register("move", map, function(){
        ...
        if (!canvasActive) {
            var cor2 = map.getExtent();
            cor2.transform(
                map.getProjectionObject(),
                new OpenLayers.Projection("EPSG:4326")
            );
            elem("#bbox_input").value = cor2.left.toFixed(6) + "," + cor2.bottom.toFixed(6) + "," + cor2.right.toFixed(6) + "," + cor2.top.toFixed(6);
        } else {
            updateBBoxInput();
        }
    });
