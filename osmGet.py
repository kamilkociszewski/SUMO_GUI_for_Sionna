...
def readCompressed(options, conn, urlpath, query, roadTypesJSON, getShapes, filename):
   ...
   <union>
    ...
    conn.request("POST", "/" + urlpath, finalQuery, headers=headers)

    response = conn.getresponse()
    if response.status != 200:
        raise RuntimeError("Overpass API returned %d %s" % (response.status, response.reason))
    if options.verbose:
...
def get_options(args):
    ...
    optParser.add_argument("--user-agent", default="osmWebWizard.py/1.0 (+https://github.com/eclipse-sumo/sumo)",
