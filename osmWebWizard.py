...
def get_sumo_home():
    ...
    # Fallback: assume it's in tools directory of SUMO_HOME
    path = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    if os.path.exists(os.path.join(path, "data", "typemap")):
        return path
    # Try typical Windows paths
    if os.name == "nt":
        for p in [r"C:\Program Files (x86)\Eclipse\Sumo", r"C:\Program Files\Eclipse\Sumo"]:
            if os.path.exists(os.path.join(p, "data", "typemap")):
                return p
    return path
...
# SUMO_HOME is already set above
DEFAULT_PORT = 8010
...
class Builder(object):
    ...
    def build(self):
        ...
        if self.data.get("coords") is None:
            ...
            mirrors = [self.data.get('osmMirror'), "www.overpass-api.de/api/interpreter", "overpass.kumi.systems/api/interpreter"]
            mirrors = [m for i, m in enumerate(mirrors) if m and m not in mirrors[:i]]
            if not mirrors:
                mirrors = ["www.overpass-api.de/api/interpreter"]

            success = False
            last_error = "Download failed"
            for mirror in mirrors:
                self.report("Downloading map data from %s" % mirror)
                osmArgs = ["-b=" + (",".join(map(str, self.data["coords"]))), "-p", self.prefix, "-d", self.tmp, "-z", "-u", mirror]
                if self.data["poly"]:
                ...
                try:
                    osmGet.get(osmArgs)
                    if os.path.exists(self.files["osm"]):
                        success = True
                        break
                except Exception as e:
                    last_error = str(e)
                    print("Error downloading from %s: %s" % (mirror, e), file=sys.stderr)

                if not os.path.exists(self.files["osm"]):
                    self.report("Download from %s failed" % mirror)
...
def main(options):
    ...
    if options.testOutputDir is not None:
        ...
        if not options.remote:
            ...
            # check for local webWizard folder in the current working directory
            local_wizard = os.path.join(os.getcwd(), "webWizard")
            if os.path.exists(os.path.join(local_wizard, "index.html")):
                path = os.getcwd()
